...
function initMap() {
  // Set a default center (update with your preferred coordinates)
  ...
  // Load store locations from stores.json
  fetch("stores.json")
    ...
    .then(stores => {
      ...
      // Also populate the sidebar list
      populateStoreList(stores);
    })
    .catch(err => console.error("Error loading stores:", err));
}

// Populate the sidebar list with store locations
function populateStoreList(stores) {
  const storeListEl = document.querySelector(".store-list");
  storeListEl.innerHTML = ""; // Clear any existing items
  stores.forEach(store => {
...
// Make initMap globally accessible (required by the Google Maps API callback)
